# fast_app.py - Fast Fake News Detection App
...
# Page configuration
st.set_page_config(
    page_title="⚡ Fast Fake News Detector",
    page_icon="⚡",
    layout="wide",
...
def load_model_and_tokenizer():
    ...
    try:
        # Load model
        model = tf.keras.models.load_model("fast_lstm_trained.h5")
...
def predict_news_fast(text, model, tokenizer):
...
def main():
    # Header
    st.markdown('<h1 class="main-header">⚡ Fast Fake News Detector</h1>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown("- ⚡ Lightweight LSTM")
        st.markdown("- 📊 20K balanced samples")
        st.markdown("- 🔤 5K vocabulary size")
        st.markdown("- 📏 200 max sequence length")
        
        st.markdown("---")
        st.markdown("**Performance:**")
        st.markdown("- ⚡ Fast prediction")
        st.markdown("- 🎯 93% accuracy")
        st.markdown("- 📈 High precision/recall")
    ...
    if model is None or tokenizer is None:
        st.error("❌ Failed to load model. Please check if 'fast_lstm_trained.h5' and 'tokenizer.pkl' exist.")
        return
        ...
        if st.button("🔍 Detect Fake News", type="primary", use_container_width=True):
            if not input_text.strip():
                ...
                with st.spinner("🔍 Analyzing..."):
                    ...
                    # Get prediction
                    label, confidence, cleaned_text = predict_news_fast(input_text, model, tokenizer)
    ...
    with col2:
        ...
        sample_texts = [
            "Scientists discover new species of dinosaur in remote mountain region",
            "Aliens spotted in downtown area, government denies everything",
            "New study shows benefits of exercise for mental health",
            "Man claims to have invented time machine in his garage"
        ]
        
        for i, sample in enumerate(sample_texts):
            if st.button(f"Sample {i+1}", key=f"sample_{i}"):
                label, confidence, _ = predict_news_fast(sample, model, tokenizer)
                st.info(f"**{label}** ({confidence:.1%} confidence)")
                st.caption(sample[:50] + "...")
        
        st.markdown("---")
        st.markdown("**Tips for better detection:**")
        st.markdown("- 📰 Include full article text")
        st.markdown("- 🎯 Provide context and sources")
        st.markdown("- ⚡ Results are instant")
        st.markdown("- 🔄 Try different samples")
